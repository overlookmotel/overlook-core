...
module.exports = coClass(class Routes extends Plugin {
	...
	*load() {
	...
	/**
	 * Get path for file of specified type for route.
	 * For use when creating new files.
	 * @param {Route} route - Route
	 * @param {string} type - Type
	 * @returns {string} - Path to file
	 */
	filePath(route, type) {
		let path = route.internalPath;
		if (path == '/') path = '/index';
		return `${path}.${this.types[type][0]}`;
	}
	...
	traverseAsync(fn, options) {
		return routerTree.traverseAsync(this.app.root, fn, options);
	}
	...
	traversePart(route, fn) {
		return routerTree.traverse(route, fn);
	}

	traverseAsyncPart(route, fn, options) {
		return routerTree.traverseAsync(route, fn, options);
	}

	flattenPart(route) {
		return routerTree.flatten(route);
	}
});
